...
void filterTest_runSquareWaveFirPowerTest(bool printMessageFlag,
  ...
  if (printMessageFlag) {
    ...
    printf(
        "running filter_runFirPowerTest() - plotting power for "
...
void filterTest_plotIirFrequencyResponse(double iirPowerValues[],
  ...
  for (uint16_t barIndex = 0; barIndex < FILTER_FREQUENCY_COUNT; barIndex++) {
    ...
    // to round the number instead of truncatFirPowerTeste.
...
#endif
  if (printMessageFlag) {
    printf("running filter_runIirPowerTest() - plotting power for all player "
           "frequencies for IIR filter(%d).\n",
           filterNumber);
  }
  double testPeriodPowerValue
