...
// 
static bool hitDetected;    // If true, a hit was detected
...
static void selectionSort(double arr[],  int indexes[], int size) {
    int i, j, max_idx;
    // 
    for (i = 0; i < size-1; i++) {
        max_idx = i;
...
bool detector_hitDetected(void) {
    ...
    int most = 0;
    int most_place = 0;
    // Use selection sorting to sor the power values
    double powerValues_sorted[FILTER_FREQUENCY_COUNT];
    // Iterate through the unsorted power array...
    for(uint32_t i = 0; i < FILTER_FREQUENCY_COUNT; i++){
        // Iterate throguh the unsorted power array from the index i...
        for(uint32_t j = i; j < FILTER_FREQUENCY_COUNT; j++){
            // Rearrange power values depending on which is greater
    ...
    // ???
    // When iterating through the unsorted vs sorted...
    // Bring outside the for loop???
    //???
    // Iterate through the sorted power values array...
    for(uint16_t i = 0; i < FILTER_FREQUENCY_COUNT; i++) {
        // If the associated frequency is not ignored...
        if (!ignoredPlayerFrequencies[i])
            // If the power value is greater than the base_line...
            if(powerValues[i] > base_line) {
                // Register the hit where needed
                hitDetected = true;
                frequencyNumberOfLastHit = i;
                detectorHitArray[i]++;
                return hitDetected;
            }
    }
